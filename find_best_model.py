...
BEST_FUTURE_PREDICTIONS_PATH = "data/future_best_preds.xlsx"  # Определение пути к файлу с лучшими прогнозами на будущее.
...
def calc_best_method_for_department(department, df, past_preds_names, past_preds_dfs):
    best_preds = []  # Создание пустого списка для хранения лучших прогнозов.

    for department in tqdm(department):  # Цикл по каждому отделу в списке department с отображением прогресса выполнения.
        y_true = list(df.loc[df["Отдел"] == department].values[0][1:])  # Получение реальных продаж для текущего отдела.
        true_sum = sum(y_true)  # Вычисление суммарных реальных продаж.

        cur_metrics = defaultdict(list)  # Создание словаря для хранения метрик ошибок для текущего отдела.
        cur_preds = []  # Создание пустого списка для хранения прогнозов для текущего отдела.
        cur_preds_sum = []  # Создание пустого списка для хранения суммарных прогнозов для текущего отдела.
        custom_metric = []  # Создание пустого списка для хранения индивидуальной метрики для текущего отдела.
        ...
        for pred_df in past_preds_dfs:
            ...
            # Вычисление метрик ошибок для текущего прогноза и добавление их в словарь метрик.
            for metric_name, metric_func in CALCULATED_METRICS.items():
                cur_metrics[metric_name].append(round(metric_func(y_true, y_pred), 2))

        idx_min_metric = np.argmin(custom_metric)  # Получение индекса минимальной индивидуальной метрики.

        # Формирование текущего лучшего прогноза, включая реальные продажи, прогнозы, модель и метрики.
        cur_best = (
        ...
        cur_best += (
            ...
            + [custom_metric[idx_min_metric]]
        )

        # Добавление вычисленных метрик ошибок в текущий лучший прогноз.
        for metric_name in CALCULATED_METRICS.keys():
            cur_best.append(cur_metrics[metric_name][idx_min_metric])

        best_preds.append(cur_best)  # Добавление текущего лучшего прогноза в общий список лучших прогнозов.

    # Создание DataFrame с лучшими прогнозами для всех товаров.
    return pd.DataFrame(
        best_preds,
        columns=list(df.columns)  # Столбцы DataFrame из исходных данных.
        + ["ModelType", "true", "pred", "diff"]  # Дополнительные столбцы для модели, реальных и прогнозируемых продаж, разницы.
        + list(CALCULATED_METRICS.keys()),  # Столбцы для вычисленных метрик ошибок.
    )
def calc_my_mape(df):
